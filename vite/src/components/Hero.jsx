...
const Hero = () => {
    const [bgFixedClass, setBgFixedClass] = useState('');

    useEffect(() => {
        //aca voy a detectar que el navegador sea ios para quitarle el bg.fixed que no es compatible 
        const esIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
        if (!esIOS) {
        setBgFixedClass('bg-fixed');
        }
    }, []);
    ...
    ;
        return ( 
            <>
                <div className={`flex relative flex-col justify-end items-center h-[101vh] text-contrast bg-[url('/escalada.webp')] bg-left lg:bg-top bg-no-repeat bg-cover ${bgFixedClass}`}>
